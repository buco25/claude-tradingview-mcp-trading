...
// EMA/RSI strategija je TRAJNO uklonjena 17.09.2026, na zahtjev (WR 10%, prevelik
// gubitak). PID string je zadrzan jer ga ULTRA-4H i glavni bot i dalje koriste u
// cross-strategy provjerama (nikad ne ulaze u simbol koji "ema_rsi" drzi) — poziva
// se samo loadPositions(EMA_RSI_PID), sto je zauvijek prazan niz sad kad strategija
// ne postoji, pa su te provjere bezopasni no-op. Povijesni tradeovi ostaju u
// trades_ema_rsi.csv za referencu, samo se vise nikad ne generiraju novi.
    ...
    for (const symbol of pDef.symbols) {
      ...
      try {
        ...
        if (pDef.strategy === "synapse_t") {
          ...
          // 28.08.: dodan `_effectiveRegime !== "BEAR"` uvjet — _btcAboveEma50 je spor 4H
          // check koji je ostajao "iznad EMA50" satima nakon što je brži 1H regime (koji
          // _effectiveRegime već preferira kad je dostupan) ispravno prešao u BEAR. Bez ovog
          // uvjeta gate je blokirao SHORT čak i kad je bot VEĆ znao da je BTC 1H bearish
          // (npr. BOUNCE MODE aktivan za LONG odskok) — to više nije divergencija, to je
          // poravnanje, pa ne treba blokirati.
          if (signal === "SHORT" && _effectiveRegime !== "BEAR" && (_effectiveRegime === "BULL" || _btcAboveEma50) && _sym1hTrend === "BEAR") {
