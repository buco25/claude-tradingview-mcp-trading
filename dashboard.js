...
function renderUltra4hSection(positions) {
  ...
  const cards = positions.map((p, idx) => {
    const uid = `u4h-${p.symbol}-${idx}`;
    const isLong = p.side === "LONG";
    const margin = p.margin ?? p.totalUSD / 20;
    return `
      <div class="pos-card ${isLong ? "pos-long" : "pos-short"}" id="u4hpos-${uid}">
    ...
    const posHtml = positions.map((p, idx) => {
      ...
      return `
        <div class="pos-card ${isLong ? "pos-long" : "pos-short"}" id="pos-${posUid}">
          ...
          <div class="pos-grid">
            <div><label>Entry</label><span>${fmtP(p.entryPrice)}</span></div>
            <div><label>SL</label><span class="red">${fmtP(p.sl)}</span>${p.entryPrice && p.sl ? `<span style="font-size:10px;color:#ff6b6b;margin-left:4px" title="Stvarna trenutna udaljenost od entrya do SL-a — SL se pomiče (BE-stop, trail) pa izvorni SL% od ulaska više ne mora vrijediti">${(Math.abs(p.entryPrice - p.sl) / p.entryPrice * 100).toFixed(2)}%</span>` : p.slPct ? `<span style="font-size:10px;color:#ff6b6b;margin-left:4px">${parseFloat(p.slPct).toFixed(2)}%</span>` : ''}${p.entryPrice && p.sl && p.quantity ? `<div style="font-size:10px;color:#ff6b6b;margin-top:1px" title="Potencijalni gubitak u dolarima ako cijena dotakne SL, po trenutnoj preostaloj količini">-$${(Math.abs(p.entryPrice - p.sl) * p.quantity).toFixed(2)}</div>` : ''}</div>
            <div><label>TP</label><span class="green">${fmtP(p.tp)}</span>${p.entryPrice && p.tp ? `<span style="font-size:10px;color:#059669;margin-left:4px" title="Stvarna trenutna udaljenost od entrya do TP-a — TP se pomiče (trail extend) pa izvorni TP% od ulaska više ne mora vrijediti">${(Math.abs(p.tp - p.entryPrice) / p.entryPrice * 100).toFixed(2)}%</span>` : p.tpPct ? `<span style="font-size:10px;color:#059669;margin-left:4px">${parseFloat(p.tpPct).toFixed(2)}%</span>` : ''}${p.entryPrice && p.tp && p.quantity ? `<div style="font-size:10px;color:#059669;margin-top:1px" title="Potencijalna dobit u dolarima ako cijena dotakne TP, po trenutnoj preostaloj količini">+$${(Math.abs(p.tp - p.entryPrice) * p.quantity).toFixed(2)}</div>` : ''}</div>
            <div><label>Notional</label><span>$${p.totalUSD.toFixed(2)}</span></div>
...
server.listen(PORT, async () => {
  ...
  setTimeout(async () => { try { await runUltra4hStrategy(); } catch (e) { console.error("ULTRA-4H strategija greška:", e.message); } }, 15000);
  setInterval(async () => {
