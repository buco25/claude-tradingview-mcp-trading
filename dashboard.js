      ...
      return `
        <div class="pos-card ${isLong ? "pos-long" : "pos-short"}" id="pos-${posUid}">
          ...
          <div class="pos-grid">
            ...
            <div><label>SL</label><span class="red">${fmtP(p.sl)}</span>${p.entryPrice && p.sl ? `<span style="font-size:10px;color:#ff6b6b;margin-left:4px" title="Stvarna trenutna udaljenost od entrya do SL-a — SL se pomiče (BE-stop, trail) pa izvorni SL% od ulaska više ne mora vrijediti">${(Math.abs(p.entryPrice - p.sl) / p.entryPrice * 100).toFixed(2)}%</span>` : p.slPct ? `<span style="font-size:10px;color:#ff6b6b;margin-left:4px">${parseFloat(p.slPct).toFixed(2)}%</span>` : ''}</div>
            <div><label>TP</label><span class="green">${fmtP(p.tp)}</span>${p.entryPrice && p.tp ? `<span style="font-size:10px;color:#059669;margin-left:4px" title="Stvarna trenutna udaljenost od entrya do TP-a — TP se pomiče (trail extend) pa izvorni TP% od ulaska više ne mora vrijediti">${(Math.abs(p.tp - p.entryPrice) / p.entryPrice * 100).toFixed(2)}%</span>` : p.tpPct ? `<span style="font-size:10px;color:#059669;margin-left:4px">${parseFloat(p.tpPct).toFixed(2)}%</span>` : ''}</div>
